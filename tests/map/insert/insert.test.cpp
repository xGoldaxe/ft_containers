...
int main( void ) {
	...
	std::pair<std::map<std::string, int>::iterator, bool> iteros = bst.insert(pair);
	std::cout << "Is inserted: " << iteros.second << std::endl; 
	std::map<std::string, int>::iterator iter = iteros.first;
	for (; iter != bst.end(); ++iter) {
		std::cout << iter->first << ":" << iter->second << std::endl;
	}

	std::cout << bst.size() << std::endl;
	for (std::map<std::string, int>::iterator it = bst.begin(); it != bst.end(); ++it) {

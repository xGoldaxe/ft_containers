...
//	self_assign = self_assign; //compiler doesn't like it!
	...
	std::vector<std::vector<int> > BOB(7, test);
	...
	std::cout << "Size of MIKE " << MIKE.size() << ";" << MIKE.capacity() << std::endl;
	std::cout << "Size of JOHN " << JOHN.size() << std::endl;
	if (JOHN.capacity() >= JOHN.size())
		std::cout << "Capacity of JOHN is sufficient\n";
	else
	...
	insert_in_me.insert(insert_in_me.begin() + 4, 2, test);
	std::cout << "================================" << std::endl; 
	for (size_t i = 0; i < insert_in_me.size(); i++)
	{
		for (size_t j = 0; j < insert_in_me.at(i).size(); j++)
	...
	//INSERT
	std::cout << "\nINSERT\n";
	std::vector<std::vector<int> >	std_insert_in_me;
	for (int i = 0; i < 15; i++)
	...
	std::vector<std::vector<int> >::iterator	std_tmp;
	...
	//INSERT RANGE
	std::cout << "\nINSERT RANGE\n";
	std::vector<std::vector<int> >	insert_bis;
	for (int i = 0; i < 7; i++)
	{
		std::vector<int>	j(2, i * 3);
	...
	//ERASE
	std::cout << "\nERASE\n";
	std::vector<std::vector<int> >	erase_in_me;
	for (int i = 0; i < 15; i++)
	{
	...
	std::cout << (MIKE == JOHN) << std::endl;
	std::cout << (JOHN == MIKE) << std::endl;
	...
	std::cout << "\nRELATIONAL OPERATORS\n";
	std::vector<std::vector<int> > MIKE_2(MIKE);
	...
	real.assign(5, test);
	for (std::vector<std::vector<int> >::iterator it = real.begin(); it != real.end(); it++)
		std::cout << (*it).front() << " ";
	std::cout << '\n';
